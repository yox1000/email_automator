...
def create_draft_email(service, to, subject, body):
    message = MIMEText(body)
    message['to'] = to
    message['subject'] = subject
    raw = base64.urlsafe_b64encode(message.as_bytes()).decode()
...
#send_email_flag parameter to switch between sending (true) and drafting (false). true is automatic
def follow_up_logic(service, send_email_flag=True):
    user_id = "me"
...
Include a reminder of the main idea in the initial proposal and express interest in hearing their thoughts on this initial proposal. Do not directly copy the original message."""
            ...
            if send_email_flag:
                ...
                create_draft_email(service, to, f"RE: {subject}", follow_up_body)

if __name__ == "__main__":
    service = authenticate()

    # Change send_email_flag to False to test with only draft creation instead of sending 
    follow_up_logic(service, send_email_flag=False)
